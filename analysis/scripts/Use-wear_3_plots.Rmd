...
## Histogram of the use-wear types with a big varaiton 
```{r}
# only use-wear type A, B, C, E
# sorts the data according to the use-wear type
sort_data_U <- imp_data[ , ] %>% arrange(Usewear.type)
# adds indices as row names 
row.names(sort_data_U) <- 1:nrow(sort_data_U)
# excludes all other use-wear types 
selected_data <- sort_data_U [c(1:21, 28:66, 109:129), ]


# plot
for (i in num.var){
  
  p_sel <- ggplot(data = selected_data, aes(x = names(imp_data)[i],fill = Usewear.type)) +
  ...
  file_out <- paste0(file_path_sans_ext(info_in[["file"]]), "_boxplot_", 
	                   names(imp_data)[i], ".pdf")
	ggsave(filename = file_out, plot = p_sel, path = dir_out, device = "pdf", width = 17,
	       height = 25, units = "cm")
...
imp_data.pca <- prcomp(imp_data[, c(24:25, 42, 47, 51, 54,56)], scale. = TRUE) 
...
a<- autoplot(imp_data.pca, data = pca, colour = "Usewear.type",
         loadings = TRUE, loadings.colour = "black", loadings.label = TRUE, loadings.label.size  = 4,  frame = TRUE, frame.type = "norm", frame.fill = "Usewear.typ")) +
         theme_classic()+
...
# using ggplot
# PCA use-wear type
b <- ggplot(pca, aes(PC1, PC2, fill = Usewear.type, addEllipses = TRUE)) +
...
# PCA tool type 
c <- ggplot(pca, aes(PC1, PC2, fill = Tool.type, colour = Tool.type)) +
